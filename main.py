...
def find_target_by_username_arg(username: str) -> Optional[TargetPlayer]:
    ...
    if not row:
        return None
    return TargetPlayer(user_id=row["user_id"], username=row["username"], display_name=row["display_name"])


def split_text_chunks(text: str, limit: int = MAX_TELEGRAM_MESSAGE) -> list[str]:
    if len(text) <= limit:
        return [text]
    chunks = []
    current = []
...
async def send_private_or_group_notice(
    ...
    if not sent and update.effective_message:
        try:
